export const foodGallery = [
  'https://images.pexels.com/photos/12737656/pexels-photo-12737656.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/9609838/pexels-photo-9609838.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1624487/pexels-photo-1624487.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1059943/pexels-photo-1059943.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/5560763/pexels-photo-5560763.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/8601417/pexels-photo-8601417.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/2474661/pexels-photo-2474661.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1640774/pexels-photo-1640774.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/825661/pexels-photo-825661.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1639562/pexels-photo-1639562.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1583884/pexels-photo-1583884.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/2347311/pexels-photo-2347311.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1907642/pexels-photo-1907642.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1437267/pexels-photo-1437267.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/761854/pexels-photo-761854.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/4109234/pexels-photo-4109234.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/2338407/pexels-photo-2338407.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/3296276/pexels-photo-3296276.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/725991/pexels-photo-725991.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/2098085/pexels-photo-2098085.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1092730/pexels-photo-1092730.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/312418/pexels-photo-312418.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/4393021/pexels-photo-4393021.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/103566/pexels-photo-103566.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1352245/pexels-photo-1352245.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/3026803/pexels-photo-3026803.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/230325/pexels-photo-230325.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/4958792/pexels-photo-4958792.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/1055272/pexels-photo-1055272.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/3026804/pexels-photo-3026804.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/723198/pexels-photo-723198.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/958545/pexels-photo-958545.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/699953/pexels-photo-699953.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/76093/pexels-photo-76093.jpeg?auto=compress&cs=tinysrgb&w=1200',
  'https://images.pexels.com/photos/67468/pexels-photo-67468.jpeg?auto=compress&cs=tinysrgb&w=1200',
];

export const categories = [
  { id: 'all', label: 'All', image: foodGallery[0] },
  { id: 'north-indian', label: 'North Indian', image: foodGallery[1] },
  { id: 'south-indian', label: 'South Indian', image: foodGallery[4] },
  { id: 'chinese', label: 'Chinese', image: foodGallery[10] },
  { id: 'pizza', label: 'Pizza', image: foodGallery[22] },
  { id: 'burger', label: 'Burger', image: foodGallery[18] },
  { id: 'biryani', label: 'Biryani', image: foodGallery[2] },
  { id: 'street-food', label: 'Street Food', image: foodGallery[9] },
  { id: 'seafood', label: 'Seafood', image: foodGallery[20] },
  { id: 'healthy', label: 'Healthy', image: foodGallery[25] },
  { id: 'wraps', label: 'Wraps', image: foodGallery[29] },
  { id: 'momos', label: 'Momos', image: foodGallery[11] },
  { id: 'thali', label: 'Thali', image: foodGallery[14] },
  { id: 'cafe', label: 'Cafe', image: foodGallery[34] },
  { id: 'desserts', label: 'Desserts', image: foodGallery[31] },
  { id: 'drinks', label: 'Drinks', image: foodGallery[35] },
  { id: 'bakery', label: 'Bakery', image: foodGallery[33] },
];

const makeMenu = (startId, restaurantName, items) =>
  items.map((item, idx) => ({
    id: startId + idx,
    restaurantId: startId,
    restaurantName,
    category: item.category,
    name: item.name,
    description: item.description,
    price: item.price,
    image: foodGallery[(startId + idx) % foodGallery.length],
  }));

const rawRestaurants = [
  {
    id: 1,
    name: 'Shiva Dhaba',
    cuisine: 'North Indian | Punjabi | Thali',
    rating: 4.4,
    deliveryTime: 18,
    deliveryFee: 0,
    minOrder: 149,
    distanceKm: 2.1,
    category: 'north-indian',
    pureVeg: true,
    offer: '50% OFF up to Rs120',
    image: foodGallery[0],
    address: 'Tips G Road, Madhuban Colony, Alwar',
    menu: makeMenu(100, 'Shiva Dhaba', [
      {
        name: 'Matar Paneer Combo',
        description: 'Matar paneer with 2 tawa roti and salad',
        price: 119,
        category: 'Combos',
      },
      {
        name: 'Dal Makhani Bowl',
        description: 'Slow-cooked black lentils with butter',
        price: 99,
        category: 'Main Course',
      },
      {
        name: 'Butter Naan',
        description: 'Soft tandoor naan finished with butter',
        price: 35,
        category: 'Breads',
      },
      {
        name: 'Lassi',
        description: 'Sweet chilled Punjabi lassi',
        price: 49,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 2,
    name: 'Wok N Roll',
    cuisine: 'Chinese | Asian | Noodles',
    rating: 4.3,
    deliveryTime: 22,
    deliveryFee: 19,
    minOrder: 199,
    distanceKm: 3.4,
    category: 'chinese',
    pureVeg: false,
    offer: 'Free spring roll on Rs399',
    image: foodGallery[10],
    address: 'Station Road, Alwar',
    menu: makeMenu(200, 'Wok N Roll', [
      {
        name: 'Hakka Noodles',
        description: 'Wok tossed noodles with veggies',
        price: 159,
        category: 'Noodles',
      },
      {
        name: 'Chilli Paneer',
        description: 'Spicy paneer cubes in garlic sauce',
        price: 189,
        category: 'Starters',
      },
      {
        name: 'Veg Manchurian',
        description: 'Crispy veg balls in manchurian gravy',
        price: 179,
        category: 'Main Course',
      },
      {
        name: 'Cold Coffee',
        description: 'Creamy cold coffee with ice',
        price: 89,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 3,
    name: 'Burger Barn',
    cuisine: 'Burgers | Fast Food | Fries',
    rating: 4.5,
    deliveryTime: 15,
    deliveryFee: 0,
    minOrder: 129,
    distanceKm: 1.9,
    category: 'burger',
    pureVeg: false,
    offer: 'Buy 1 Get 1 on burgers',
    image: foodGallery[18],
    address: 'Scheme 2, Alwar',
    menu: makeMenu(300, 'Burger Barn', [
      {
        name: 'Classic Chicken Burger',
        description: 'Juicy chicken patty with lettuce and cheese',
        price: 169,
        category: 'Burger',
      },
      {
        name: 'Veg Cheese Burger',
        description: 'Crispy veg patty with cheddar',
        price: 149,
        category: 'Burger',
      },
      {
        name: 'Peri Peri Fries',
        description: 'Crispy fries tossed in peri peri seasoning',
        price: 109,
        category: 'Sides',
      },
      {
        name: 'Chocolate Shake',
        description: 'Thick chocolate shake topped with cream',
        price: 119,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 4,
    name: 'Pizza Piazza',
    cuisine: 'Pizza | Italian | Pasta',
    rating: 4.2,
    deliveryTime: 27,
    deliveryFee: 29,
    minOrder: 249,
    distanceKm: 4.2,
    category: 'pizza',
    pureVeg: false,
    offer: 'Flat Rs150 OFF on medium pizza',
    image: foodGallery[23],
    address: 'Moti Doongri, Alwar',
    menu: makeMenu(400, 'Pizza Piazza', [
      {
        name: 'Margherita Pizza',
        description: 'Classic cheese pizza with basil',
        price: 219,
        category: 'Pizza',
      },
      {
        name: 'Farmhouse Pizza',
        description: 'Onion, capsicum, tomato, mushroom',
        price: 299,
        category: 'Pizza',
      },
      {
        name: 'White Sauce Pasta',
        description: 'Creamy alfredo pasta with herbs',
        price: 229,
        category: 'Pasta',
      },
      {
        name: 'Garlic Bread',
        description: 'Buttery garlic bread with dip',
        price: 119,
        category: 'Sides',
      },
    ]),
  },
  {
    id: 5,
    name: 'Dosa Factory',
    cuisine: 'South Indian | Tiffin | Breakfast',
    rating: 4.6,
    deliveryTime: 20,
    deliveryFee: 9,
    minOrder: 99,
    distanceKm: 2.7,
    category: 'south-indian',
    pureVeg: true,
    offer: 'Combo from Rs129',
    image: foodGallery[4],
    address: 'Bus Stand Road, Alwar',
    menu: makeMenu(500, 'Dosa Factory', [
      {
        name: 'Masala Dosa',
        description: 'Crispy dosa with potato masala filling',
        price: 119,
        category: 'Dosa',
      },
      {
        name: 'Idli Sambar',
        description: 'Steamed idlis with hot sambar and chutney',
        price: 99,
        category: 'Idli',
      },
      {
        name: 'Onion Uttapam',
        description: 'Soft uttapam topped with onions',
        price: 129,
        category: 'Uttapam',
      },
      {
        name: 'Filter Coffee',
        description: 'South-style aromatic filter coffee',
        price: 69,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 6,
    name: 'Biryani House',
    cuisine: 'Biryani | Kebab | Mughlai',
    rating: 4.1,
    deliveryTime: 28,
    deliveryFee: 19,
    minOrder: 179,
    distanceKm: 4.8,
    category: 'biryani',
    pureVeg: false,
    offer: 'Rs75 OFF on first order',
    image: foodGallery[2],
    address: 'Hope Circus, Alwar',
    menu: makeMenu(600, 'Biryani House', [
      {
        name: 'Chicken Dum Biryani',
        description: 'Aromatic basmati with spiced chicken',
        price: 249,
        category: 'Biryani',
      },
      {
        name: 'Veg Biryani',
        description: 'Mixed veg biryani with raita',
        price: 199,
        category: 'Biryani',
      },
      {
        name: 'Mutton Curry',
        description: 'Slow-cooked mutton in rich gravy',
        price: 329,
        category: 'Main Course',
      },
      {
        name: 'Kebab Platter',
        description: 'Assorted kebabs with green chutney',
        price: 289,
        category: 'Starters',
      },
    ]),
  },
  {
    id: 7,
    name: 'Salad Story',
    cuisine: 'Healthy | Salads | Bowls',
    rating: 4.7,
    deliveryTime: 16,
    deliveryFee: 0,
    minOrder: 149,
    distanceKm: 1.4,
    category: 'healthy',
    pureVeg: true,
    offer: 'Healthy bowl at Rs99',
    image: foodGallery[25],
    address: 'City Center Mall, Alwar',
    menu: makeMenu(700, 'Salad Story', [
      {
        name: 'Caesar Salad',
        description: 'Crunchy lettuce, croutons, creamy dressing',
        price: 169,
        category: 'Salads',
      },
      {
        name: 'Greek Salad',
        description: 'Tomato, cucumber, olives, feta',
        price: 179,
        category: 'Salads',
      },
      {
        name: 'Falafel Wrap',
        description: 'Crispy falafel, hummus, fresh veggies',
        price: 189,
        category: 'Wraps',
      },
      {
        name: 'Mango Smoothie',
        description: 'Fresh mango smoothie with yogurt',
        price: 109,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 8,
    name: 'Sweet Cravings',
    cuisine: 'Desserts | Bakery | Ice Cream',
    rating: 4.8,
    deliveryTime: 19,
    deliveryFee: 15,
    minOrder: 99,
    distanceKm: 3.1,
    category: 'desserts',
    pureVeg: true,
    offer: '20% OFF on cakes',
    image: foodGallery[31],
    address: 'Nehru Circle, Alwar',
    menu: makeMenu(800, 'Sweet Cravings', [
      {
        name: 'Chocolate Truffle Cake',
        description: 'Rich chocolate sponge with ganache',
        price: 299,
        category: 'Cakes',
      },
      {
        name: 'Hot Brownie Sundae',
        description: 'Warm brownie with vanilla scoop',
        price: 199,
        category: 'Desserts',
      },
      {
        name: 'Donut Box',
        description: 'Assorted glazed donuts (4 pcs)',
        price: 179,
        category: 'Desserts',
      },
      {
        name: 'Waffle with Nutella',
        description: 'Crispy waffle topped with Nutella',
        price: 189,
        category: 'Desserts',
      },
    ]),
  },
  {
    id: 9,
    name: 'Coastal Catch',
    cuisine: 'Seafood | Coastal | Grill',
    rating: 4.0,
    deliveryTime: 33,
    deliveryFee: 39,
    minOrder: 299,
    distanceKm: 5.5,
    category: 'seafood',
    pureVeg: false,
    offer: 'Free delivery above Rs499',
    image: foodGallery[20],
    address: 'Siliserh Road, Alwar',
    menu: makeMenu(900, 'Coastal Catch', [
      {
        name: 'Fish Fry',
        description: 'Crispy fish fillet with lemon butter',
        price: 269,
        category: 'Seafood',
      },
      {
        name: 'Prawn Curry',
        description: 'Coconut based spicy prawn curry',
        price: 319,
        category: 'Seafood',
      },
      {
        name: 'Grilled Fish Rice',
        description: 'Grilled fish served with herbed rice',
        price: 299,
        category: 'Combos',
      },
      {
        name: 'Lemon Mojito',
        description: 'Fresh mint lemon cooler',
        price: 99,
        category: 'Drinks',
      },
    ]),
  },
  {
    id: 10,
    name: 'Chaat Bazaar',
    cuisine: 'Street Food | Snacks | Chaat',
    rating: 4.3,
    deliveryTime: 14,
    deliveryFee: 0,
    minOrder: 79,
    distanceKm: 1.2,
    category: 'street-food',
    pureVeg: true,
    offer: 'Under Rs99 specials',
    image: foodGallery[9],
    address: 'Clock Tower Market, Alwar',
    menu: makeMenu(1000, 'Chaat Bazaar', [
      {
        name: 'Pav Bhaji',
        description: 'Spiced mashed veggies with butter pav',
        price: 99,
        category: 'Street Food',
      },
      {
        name: 'Aloo Tikki Chaat',
        description: 'Crispy aloo tikki with chutneys and curd',
        price: 89,
        category: 'Street Food',
      },
      {
        name: 'Dahi Puri',
        description: 'Crunchy puris filled with curd and chutney',
        price: 79,
        category: 'Street Food',
      },
      {
        name: 'Masala Chaas',
        description: 'Refreshing spiced buttermilk',
        price: 49,
        category: 'Drinks',
      },
    ]),
  },
];

const extraRestaurantSeeds = [
  { name: 'Royal Tandoor', cuisine: 'North Indian | Tandoori', category: 'north-indian', pureVeg: false, offer: 'Flat Rs100 OFF', area: 'Lajpat Nagar' },
  { name: 'Spice Junction', cuisine: 'Indian | Curry | Combo', category: 'north-indian', pureVeg: false, offer: 'Buy 2 Get 1 Free', area: 'Nangli Circle' },
  { name: 'Noodle Nest', cuisine: 'Chinese | Noodles | Momos', category: 'chinese', pureVeg: false, offer: '20% OFF on combos', area: 'Railway Station Road' },
  { name: 'Burger Blast', cuisine: 'Burgers | Fries | Wraps', category: 'burger', pureVeg: false, offer: 'Free fries on Rs299', area: 'Scheme 10' },
  { name: 'Cheese Crust', cuisine: 'Pizza | Pasta | Garlic Bread', category: 'pizza', pureVeg: false, offer: 'Second pizza at Rs99', area: 'Moti Chowk' },
  { name: 'Veggie Delight', cuisine: 'Healthy | Salad | Bowl', category: 'healthy', pureVeg: true, offer: 'Healthy meal from Rs129', area: 'Shastri Nagar' },
  { name: 'Desi Rasoi', cuisine: 'Pure Veg | Thali | Indian', category: 'thali', pureVeg: true, offer: 'Thali combo special', area: 'Old Market' },
  { name: 'Dum Biryani Co', cuisine: 'Biryani | Mughlai | Kebabs', category: 'biryani', pureVeg: false, offer: 'Rs125 OFF on family pack', area: 'Hasan Khan Mewati Nagar' },
  { name: 'Momo Point', cuisine: 'Tibetan | Chinese | Fast Food', category: 'momos', pureVeg: false, offer: 'Free dip with momos', area: 'Company Bagh' },
  { name: 'Waffle Works', cuisine: 'Desserts | Waffles | Shakes', category: 'desserts', pureVeg: true, offer: 'Sweet combo at Rs149', area: 'Clock Tower' },
  { name: 'Coffee Crew', cuisine: 'Cafe | Coffee | Snacks', category: 'cafe', pureVeg: true, offer: 'Buy 1 Get 1 Coffee', area: 'Kala Kuan' },
  { name: 'Punjabi Zaika', cuisine: 'Punjabi | Kebabs | Curry', category: 'north-indian', pureVeg: false, offer: '40% OFF up to Rs120', area: 'Surya Nagar' },
  { name: 'Street Bites', cuisine: 'Street Food | Chaat | Rolls', category: 'street-food', pureVeg: true, offer: 'Under Rs99 specials', area: 'Nehru Garden' },
  { name: 'Sizzler Hub', cuisine: 'Continental | Grill | Sizzler', category: 'burger', pureVeg: false, offer: 'Starter free on Rs499', area: 'Scheme 2' },
  { name: 'Pasta Palace', cuisine: 'Italian | Pasta | Pizza', category: 'pizza', pureVeg: true, offer: 'Cheese burst deal', area: 'Kashiram Circle' },
  { name: 'The Bowl House', cuisine: 'Healthy | Bowl | Smoothie', category: 'healthy', pureVeg: true, offer: 'Flat Rs80 OFF', area: 'Naman Hotel Road' },
  { name: 'Tikka Town', cuisine: 'BBQ | Tikka | Rolls', category: 'north-indian', pureVeg: false, offer: 'Family pack offer', area: 'Hope Circus' },
  { name: 'Chai & Snacks', cuisine: 'Snacks | Tea | Pakoda', category: 'north-indian', pureVeg: true, offer: 'Evening special combo', area: 'Station Area' },
  { name: 'South Spice', cuisine: 'South Indian | Dosa | Idli', category: 'south-indian', pureVeg: true, offer: 'Breakfast combo from Rs89', area: 'Gandhi Nagar' },
  { name: 'Wrap It Up', cuisine: 'Wraps | Roll | Fast Food', category: 'wraps', pureVeg: false, offer: 'Free drink on wraps', area: 'Rajasthan Circle' },
  { name: 'Donut Den', cuisine: 'Donuts | Dessert | Bakery', category: 'bakery', pureVeg: true, offer: '2+1 donut box', area: 'Bala Quila Road' },
  { name: 'Kebab Kingdom', cuisine: 'Kebab | Mughlai | Curry', category: 'north-indian', pureVeg: false, offer: 'Late night special', area: 'MIA Road' },
  { name: 'Sushi Spot', cuisine: 'Japanese | Sushi | Ramen', category: 'chinese', pureVeg: false, offer: 'Combo platter at Rs299', area: 'City Mall' },
  { name: 'Fresh & Fit', cuisine: 'Salad | Juice | Detox', category: 'healthy', pureVeg: true, offer: 'Clean eating offer', area: 'Shopex Mall' },
  { name: 'Thali Express', cuisine: 'North Indian | Thali | Combo', category: 'thali', pureVeg: true, offer: 'Thali deal from Rs119', area: 'Manu Marg' },
  { name: 'Roll Republic', cuisine: 'Rolls | Wraps | Fast Food', category: 'wraps', pureVeg: false, offer: 'Flat Rs90 OFF', area: 'Matsya Nagar' },
  { name: 'Maharaja Meals', cuisine: 'Indian | Curry | Meals', category: 'north-indian', pureVeg: false, offer: 'Meal combo special', area: 'Budh Vihar' },
  { name: 'Dosa Adda', cuisine: 'South Indian | Dosa | Tiffin', category: 'south-indian', pureVeg: true, offer: 'Masala dosa from Rs89', area: 'Nai Mandi' },
  { name: 'Tandoori Treat', cuisine: 'Tandoor | Kebabs | Grill', category: 'north-indian', pureVeg: false, offer: '30% OFF on grills', area: 'Arya Nagar' },
  { name: 'Pizza Junction', cuisine: 'Pizza | Pasta | Italian', category: 'pizza', pureVeg: false, offer: 'Cheese combo at Rs199', area: 'Kalakand Market' },
  { name: 'Snack Station', cuisine: 'Snacks | Chaat | Tea', category: 'north-indian', pureVeg: true, offer: 'Evening snacks combo', area: 'Neemrana Road' },
  { name: 'Mughlai Nights', cuisine: 'Mughlai | Biryani | Korma', category: 'north-indian', pureVeg: false, offer: 'Rs150 OFF on family order', area: 'Ambedkar Nagar' },
  { name: 'Cake Studio', cuisine: 'Desserts | Cakes | Bakery', category: 'bakery', pureVeg: true, offer: 'Free pastry on Rs399', area: 'Bhagat Singh Circle' },
];

const extraRestaurants = extraRestaurantSeeds.map((seed, index) => {
  const base = rawRestaurants[index % rawRestaurants.length];
  const restaurantId = 11 + index;
  const menuBaseId = 2000 + index * 10;

  return {
    ...base,
    id: restaurantId,
    name: seed.name,
    cuisine: seed.cuisine,
    category: seed.category,
    pureVeg: seed.pureVeg,
    offer: seed.offer,
    rating: Number((4 + ((index * 7) % 9) / 10).toFixed(1)),
    deliveryTime: 14 + (index % 18),
    deliveryFee: index % 3 === 0 ? 0 : 9 + (index % 5) * 5,
    minOrder: 99 + (index % 6) * 30,
    distanceKm: Number((1.1 + (index % 8) * 0.6).toFixed(1)),
    image: foodGallery[(index + 11) % foodGallery.length],
    address: `${seed.area}, Alwar`,
    city: 'Alwar',
    menu: base.menu.map((item, menuIndex) => ({
      ...item,
      id: menuBaseId + menuIndex,
      restaurantId,
      restaurantName: seed.name,
      image: foodGallery[(index * 3 + menuIndex + 5) % foodGallery.length],
    })),
  };
});

const allRestaurants = [...rawRestaurants, ...extraRestaurants];
const cityPool = ['Alwar', 'Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai'];
const priceRanges = ['Rs150 for one', 'Rs200 for one', 'Rs250 for one', 'Rs300 for one'];

export const restaurants = allRestaurants.map((restaurant, index) => ({
  ...restaurant,
  city: restaurant.city || cityPool[index % cityPool.length],
  priceRange: restaurant.priceRange || priceRanges[index % priceRanges.length],
}));

export const promoBanners = [
  {
    id: 1,
    title: 'Meals Under Rs250',
    subtitle: 'Final price, best offer applied',
    image: foodGallery[3],
  },
  {
    id: 2,
    title: 'Late Night Cravings',
    subtitle: 'Open till 2 AM near you',
    image: foodGallery[16],
  },
  {
    id: 3,
    title: 'Dessert Weekend',
    subtitle: 'Sweet deals up to 40% OFF',
    image: foodGallery[30],
  },
];

export const getRestaurantById = (id) =>
  restaurants.find((restaurant) => restaurant.id === Number(id));
